...
interface WeeklyRevenue {
  start: string; // fecha de inicio de la semana
  end: string;   // fecha de fin de la semana
  revenue: number;
}
...
interface RevenueAnalytics {
  weeklyAnalytics: WeeklyRevenue[]; // todas las semanas calculadas
  monthlyAnalytics: MonthlyRevenue[]; // últimos 3 meses
}
...
export const OrderProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // Aquí nos aseguramos de que el formato coincida con nuestro context
      setAnalytics({
