...
export default function BestSellers() {
  ...
  return (
    <Card classtitle="p-6 max-w-[500px] ">
      <CardHeader >
       
         <CardTitle classtitle="font-bold text-lg  ">Best Sellers</CardTitle>
        <CardDescription>Last 6 months</CardDescription>
       
        <div classtitle='flex justify-between items-center'>
          <CardDescription>Top 10</CardDescription>

        
        <div classtitle="my-2 w-48">
      ...
      <CardContent>
        <ul classtitle="text-sm text-slate-600 mt-4">
          {topData.map((item) => (
            <li key={item.title} classtitle="flex justify-between py-1">
              <span>{item.title}</span>
              <span classtitle="font-medium">{item.quantity}</span>
            </li>
          ))}
        </ul>
      </CardContent>
