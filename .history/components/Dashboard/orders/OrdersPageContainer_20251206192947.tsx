                ...
                <td className="p-2">
                  <StatusDropdown
                    ...
                    onChange={async (newStatus) => {
                      setLocalStatuses(prev => ({ ...prev, [order._id]: newStatus })); // actualiza UI de inmediato
                      try {
                        await updateOrderStatus(order._id, newStatus); // actualiza DB
                      } catch (err) {
                        console.error(err);
                        setLocalStatuses(prev => ({ ...prev, [order._id]: order.status })); // revertir si falla
                      }
                    }}
                  />
                </td>
      ...
      {modalOpen && selectedOrder && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-3/4 max-w-2xl">
            ...
            {/* Renderiza aquí los productos, cantidades, tallas, método de envío, etc */}
            <button
