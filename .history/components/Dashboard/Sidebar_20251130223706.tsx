...
const Sidebar: FC<SidebarProps> = ({
  ...
  return (
    <div
      className={`h-screen overflow-y-auto pb-10  dark:bg-card shadow-md transition-all duration-300 
      ${collapsed ? "w-20" : "w-64"}`}
    >
      <div className="flex justify-between items-center p-4">
        {!collapsed && (
          ...
          >
            <span className="text-white">Hancrated</span>
          </Link>
        )}
      </div>
      ...
      <div className="mt-5">
        {sidebarLinks.map((section) => (
          <div key={section.title}>
            {!collapsed && (
              <p className="text-white uppercase px-4 mt-4 mb-2 text-sm">
                {section.title}
              </p>
            )}
                ...
                <Link
                  ...
                  className={`flex items-center gap-4 pl-4 py-3 rounded-lg m-2 transition-all duration-300
                   ${
                     collapsed
                       ? "justify-center"
                       : "text-white dark:text-gray-200 hover:bg-gray-100"
                   }`}
                >
                  <Icon size={20} />
                  {!collapsed && (
