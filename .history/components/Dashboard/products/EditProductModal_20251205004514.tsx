...
export default function EditProductModal({
  ...
  // Siempre inicializamos arrays y quantity
  useEffect(() => {
    if (product) {
      setForm({
        ...product,
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl p-6 w-[90%] max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
        ...
        <div className="mb-4">
          ...
          {(form.images || []).map((img, index) => (
            <div key={index} className="relative w-20 h-20 rounded overflow-hidden border border-slate-300 dark:border-slate-600">
              {img ? (
                <Image
                  src={img}
                  alt={`Product ${index}`}
              ...
              <button onClick={() => removeImage(index)} className="absolute top-1 right-1 bg-black/70 text-white rounded-full p-1">
                <X size={14} />
              </button>
            </div>
          ))}
