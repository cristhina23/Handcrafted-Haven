...
export default function AddProductFormExtended() {
  ...
  // Añadir o quitar método de envío
  ...
  return (
    <form className="max-w-3xl mx-auto p-6 bg-gray-900 rounded-lg text-white space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="flex items-center gap-2">
        <input type="checkbox" checked={isCustomOrder} onChange={(e) => setIsCustomOrder(e.target.checked)} />
      ...
      <div className="space-y-2 border border-dashed p-4 rounded">
        ...
        <input type="file" accept="image/*" onChange={(e) => e.target.files && setMainImage(e.target.files[0])} />
        {mainImage && (
  <div className="flex items-center gap-2 mt-2 relative">
    ...
    <button
      type="button"
      onClick={() => setMainImage(null)} // elimina la imagen principal
    ...
    >
      ...
      <div className="space-y-2 border border-dashed p-4 rounded">
        ...
        <input type="file" accept="image/*" multiple onChange={(e) => e.target.files && setGalleryImages(Array.from(e.target.files))} />
  ...
  {galleryImages.map((file, idx) => (
    <div key={idx} className="relative">
      ...
      <button
        type="button"
        onClick={() => {
          // elimina la imagen de la galería completamente
          const newGallery = galleryImages.filter((_, i) => i !== idx);
          setGalleryImages(newGallery);
        }}
        className="absolute -top-2 -right-2 bg-red-600 rounded-full p-1 text-white text-xs"
      >
        ✕
      </button>
    </div>
