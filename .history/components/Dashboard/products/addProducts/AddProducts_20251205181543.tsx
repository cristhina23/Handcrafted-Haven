    ...
    <form className="max-w-3xl mx-auto p-6 bg-gray-900 rounded-lg text-white space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="space-y-2 border border-dashed p-4 rounded">
        <div className="space-y-2 ">
          <label className="relative w-40 h-40 border border-dashed rounded flex items-center justify-center cursor-pointer border-slate-400 dark:border-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 transition">
            {mainImage ? (
              <Image
                width={100}
                height={100}
          ...
          {mainImage && (
            <button
              ...
              className="bg-red-600 absolute top-2 right-2 rounded-full text-white px-2 py-1 rounded"
            >
              x
            </button>
          )}
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {galleryImages.map((file, idx) => (
            <div key={idx} className="relative">
              ...
              <Button type="button" variant="destructive" size="icon" onClick={() => removeGalleryImage(idx)}>✕</Button>
            </div>
          ))}
        </div>
      </div>
