...
export default function MyStorePage() {
  ...
  try {
    let uploadedImageUrl = form.profileImage; // URL actual por defecto

    // Si hay un archivo nuevo, subirlo a tu API
    if (localImage) {
      ...
      uploadedImageUrl = data.url; // guardamos la URL subida
    }
    ...
    setEdit(false);
    setLocalImage(null); // limpiar archivo local
  } catch (err) {
    console.error(err);
  }
}
...
<div className="flex flex-col items-center justify-center gap-4 mb-6 relative">
  <div className="relative w-28 h-28 rounded-md overflow-hidden border shadow">
    <Image
      ...
      <div className="absolute top-0 right-0">
        {localImage ? (
          <button
            onClick={() => {
              setLocalImage(null);
              setForm((prev) => ({ ...prev, profileImage: seller.profileImage || "" }));
            }}
