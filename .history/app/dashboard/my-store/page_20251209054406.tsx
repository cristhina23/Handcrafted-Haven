...
const handleSubmit = async () => {
  let profileImageUrl = form.profileImage; // imagen actual por defecto

  // 1) Si el usuario seleccionó una imagen nueva, subirla
    if (form.newProfileImageFile) {
      const imgData = new FormData();
      imgData.append("image", form.newProfileImageFile);

      const uploadRes = await fetch("/api/seller/update-image", {
        method: "POST",
        body: imgData,
      });
      ...
      profileImageUrl = url; // nueva imagen subida
    }

    // 2) Actualizar el resto del store
