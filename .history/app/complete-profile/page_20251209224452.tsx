...
export default function CompleteProfilePage() {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        if (data.exists && data.user) {
          // User exists in DB, pre-populate with their data
          const dbUser = data.user;

          // Fetch full user data to get address
          ...
          if (userResponse.ok) {
            ...
            let phoneNumber = "";
            if (userData.phone) {
              const phoneParts = userData.phone.match(/^(\+\d+)\s*(.+)$/);
              if (phoneParts) {
            ...
            setFormData({
              username: userData.username || clerkUsername,
              countryCode,
              phoneNumber,
    ...
    loadUserData();
  }, [user?.id, clerkUsername]);

  // Actualizar username cuando el usuario de Clerk cargue (solo si no hay data de DB)
  useEffect(() => {
    if (clerkUsername && !formData.username && !loadingProfile) {
      setFormData((prev) => ({ ...prev, username: clerkUsername }));
    }
  }, [clerkUsername, formData.username, loadingProfile]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F9F8F6] py-12 px-4">
      <Card className="w-full max-w-2xl">
        ...
        <CardContent>
          {loadingProfile ? (
            <div className="text-center py-8 text-[#876A5C]">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={handleChange}
                        className="h-9 w-full rounded-md border border-[#D0D3D8] bg-white px-3 text-sm shadow-xs focus:border-slate-800 focus:outline-none focus:ring-2 focus:ring-[#876A5C]/20"
                      >
                        {countryCodes.map((item) => (
