...
export default function CreateShopPage() {
  ...
  const [sellerImageUrl, setSellerImageUrl] = useState<string>("");

  const [loading, setLoading] = useState(false);

  // -----------------------------
  // 1️⃣ Subir imagen AUTOMÁTICAMENTE
  // -----------------------------
  const handleSellerImageUpload = async (file: File) => {
    setTempSellerImage(file); // preview
    ...
    setSellerImageUrl(data.url); // URL final de Cloudinary
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const body = {
      ...
      profileImage: sellerImageUrl, // SOLO se envía la URL
    };
  ...
  return (
    <div className="max-w-2xl mx-auto mdmy-8 py-10 px-4 text-slate-900 bg-white rounded-xl md:px-8 border border-gray-200 shadow-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex flex-col items-center gap-3 ">
          <div className="relative w-40 h-40">
            <label className="w-40 h-40 border border-dashed rounded-full flex items-center justify-center cursor-pointer border-slate-400 hover:bg-slate-200 transition">
              ...
              <input
                ...
                onChange={(e) => {
                  if (e.target.files?.[0]) {
                    handleSellerImageUpload(e.target.files[0]);
                  }
                }}
              />
            </label>
          ...
          <p className="text-sm text-slate-700">
            {sellerImageUrl ? "Image uploaded ✔" : "Upload your profile picture"}
          </p>
        </div>
