...
export default function CreateShopPage() {
  ...
  // ⛔ Ya NO sube imagen aquí, solo guarda el file
  ...
  // 🔥 Ahora la imagen se sube aquí, al enviar el formulario
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1️⃣ SUBIR IMAGEN AQUÍ
    const uploadedImageUrl = await uploadImageOnSubmit();

    // 2️⃣ FORMATEAR SPECIALTIES
    const specialtiesArray = specialties
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);

    // 3️⃣ ENVIAR DATOS AL SERVER
    const body = {
      shopName,
      bio,
      country,
      specialties: specialtiesArray,
      profileImage: uploadedImageUrl, // ahora sí existe correctamente
    };
