...
export async function GET() {
  try {
    ...
    sellerOrdersItems.forEach(order => {
    ...
    const totalOrders = sellerOrdersItems.length;
    const totalRevenue = sellerOrdersItems.reduce((sum, order) => sum + (order.grandTotal || 0), 0);
    const totalCustomers = new Set(sellerOrdersItems.map(order => order.buyerId?.toString() || "")).size;
