...
interface SellerOrderItem {
  orderId: string;         // order._id como string
  buyerId: string;         // buyerId como string
  subtotal: number;
  createdAt: Date;
}
...
export async function GET() {
  try {
    ...
    // Filtramos los items que pertenecen al seller
    ...
    // Función helper para obtener el inicio de la semana (lunes)
    ...
    // Calculamos ingresos semanales
    const startDate = new Date(now.getFullYear(), now.getMonth() - 2, 1); // 3 meses atrás
    ...
    // Calculamos ingresos mensuales de los últimos 3 meses
